...
HRESULT singo::init(void)
{
	...
	this->motionInit(WALK_RIGHT, L"Singo_Right_Forward", false, true, 8);
	this->motionInit(WALK_LEFT, L"Singo_Left_Forward", false, true, 8);
	this->motionInit(BACKWALK_RIGHT, L"Singo_Right_Retreat", false, true, 8);
	this->motionInit(BACKWALK_LEFT, L"Singo_Left_Retreat", false, true, 8);
	this->motionInit(DASH_RIGHT, L"Singo_Right_Run", false, true, 10);
	this->motionInit(DASH_LEFT, L"Singo_Left_Run", false, true, 6);
	...
	this->motionInit(PUNCH1_RIGHT, L"Singo_Right_StandPunch1", false, true, 9);
	...
	//_rcDamage = RectMake(_x, _y, 100, 100);
...
void singo::update(void)
{
	...
	switch (_pos)
	{
		...
		case player::PUNCH1_RIGHT:
			_rcDamage = RectMake(_x , _y - 250, 110, 60);
			if (!_ani[_pos].isPlay()) _rcDamage = RectMake(0, 0, 0, 0);
		break;
		case player::PUNCH2_RIGHT:
			_rcDamage = RectMake(_x , _y - 250, 150, 150);
			if (!_ani[_pos].isPlay()) _rcDamage = RectMake(0, 0, 0, 0);
		break;
		case player::JUMP_PUNCH1_RIGHT:
			_rcDamage = RectMake(_x, _y - 150, 110, 150);
			if (!_ani[_pos].isPlay()) _rcDamage = RectMake(0, 0, 0, 0);
		break;
		...
		case player::KICK1_RIGHT:
			_rcDamage = RectMake(_x, _y - 210, 180, 60);
			if (!_ani[_pos].isPlay()) _rcDamage = RectMake(0, 0, 0, 0);
		break;
		case player::KICK2_RIGHT:
			_rcDamage = RectMake(_x, _y - 270, 160, 150);
			if (!_ani[_pos].isPlay()) _rcDamage = RectMake(0, 0, 0, 0);
		break;
...
void singo::render(void)
{
	...
	Rectangle(getMemDC(), _rcDamage.left, _rcDamage.top, _rcDamage.right, _rcDamage.bottom);
...
void singo::control()
{
	...
	if ((KEYMANAGER->isOnceKeyUp(VK_RIGHT) || KEYMANAGER->isOnceKeyUp(VK_LEFT)
		|| KEYMANAGER->isOnceKeyUp(VK_DOWN)) && _pos != PUNCH1_RIGHT && _pos != PUNCH2_RIGHT &&
	...
	if (KEYMANAGER->isStayKeyDown(VK_LEFT) && _pos != PUNCH1_RIGHT && _pos != PUNCH2_RIGHT &&
		_pos != JUMP_PUNCH1_RIGHT && _pos != JUMP_PUNCH2_RIGHT && _pos != KICK1_RIGHT && _pos != KICK2_RIGHT &&
		_pos != JUMP_KICK1_RIGHT && _pos != JUMP_KICK2_RIGHT)
	{
		_x -= PLAYER_SPEED - 2;
	...
	if (KEYMANAGER->isOnceKeyDown(VK_UP) && _pos != PUNCH1_RIGHT && _pos != PUNCH2_RIGHT &&
		_pos != JUMP_PUNCH1_RIGHT && _pos != JUMP_PUNCH2_RIGHT && _pos != KICK1_RIGHT && _pos != KICK2_RIGHT &&
		_pos != JUMP_KICK1_RIGHT && _pos != JUMP_KICK2_RIGHT)
	{
		_pos = JUMP_MID;
